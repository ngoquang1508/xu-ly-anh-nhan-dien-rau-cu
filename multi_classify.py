...
DB_PATH = os.path.join(BASE_DIR, "vegetable_database.json")

DATASET_PATH = os.path.join(BASE_DIR, "..", "Vegetable Images")
...
def extract_vector(img):
    ...
    yellow_ratio = np.mean((r > 150) & (g > 150))

    color_bonus = np.array([
        np.mean((g>r+20)&(g>b+20)) * 4,
        np.mean((r>g+40)&(r>b+40)) * 4,
        yellow_ratio * 3
    ])
